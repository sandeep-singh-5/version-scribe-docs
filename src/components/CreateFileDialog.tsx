...
const CreateFileDialog = ({ onFileCreate }: CreateFileDialogProps) => {
  ...
  useEffect(() => {
    if (showEditor && quillRef.current && !quillInstance.current) {
  ...
  const handleSaveDocument = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${fileData.fileName} uploaded successfully`,
      });
      resetForm();
    } catch (err) {
      toast({
  ...
  return (
    ...
    >
      ...
      <DialogContent
        className={`${
          showEditor
            ? "sm:max-w-6xl w-full max-w-[95vw] h-[90vh]"
            : "sm:max-w-2xl w-full max-w-[95vw]"
        } bg-card/95 backdrop-blur-sm border-border shadow-lg overflow-y-auto`}
      >
        <DialogHeader className="pb-4 border-b border-border/50">
          <DialogTitle className="text-xl font-bold bg-gradient-primary bg-clip-text text-transparent">
            {showEditor
              ? `Word Editor - ${formData.fileName}.docx`
        ...
        {showEditor ? (
          <div className="flex flex-col h-full">
            <div className="flex-1 p-2 sm:p-4 bg-muted/30 rounded-xl border border-border/50 overflow-hidden">
              <div
                ref={quillRef}
                className="h-full min-h-[60vh] bg-background rounded-lg border border-border"
              />
            </div>
            <div className="flex flex-col sm:flex-row justify-end gap-3 mt-6 pt-4 border-t border-border/50">
          ...
          <div className="space-y-6">
            ...
            >
              <TabsList className="grid w-full grid-cols-2 bg-muted/30 p-1 h-12">
                <TabsTrigger
                  value="create"
                  className="h-10 data-[state=active]:bg-gradient-primary data-[state=active]:text-primary-foreground"
                >
              ...
              >
                <div className="p-6 border-2 border-dashed rounded-xl bg-muted/20 text-center cursor-pointer hover:bg-muted/30 transition">
